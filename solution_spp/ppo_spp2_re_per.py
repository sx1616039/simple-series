...
from job_img_gray import JobEnv
...
class Actor_SPP(nn.Module):
    def __init__(self, input_channel=2, out_channel=4, out_num=6, num_level=4):
        super(Actor_SPP, self).__init__()
        self.num_level = num_level
        self.num_grid = _cal_num_grids(num_level)
        self.feature1 = nn.Sequential(nn.Conv2d(input_channel, out_channel, kernel_size=(5, 5), padding=2), nn.Tanh())
...
class Critic_SPP(nn.Module):
    def __init__(self, input_channel=2, out_channel=4, out_num=1, num_level=4):
    ...
    def update(self, bs, ba, br, bp):
        ...
        for i in range(self.UPDATE_STEPS):
            ...
            for r in range(self.PER_NUM):
                ...
                indices = np.random.choice(len(prob1), self.replay_size, p=prob1)
                if self.beta < self.upper_bound:
                    ...
                    # self.priorities[indices] = self.learn(state[indices], action[indices], d_reward[indices],
                    #                                       old_log_prob[indices], weights)
                else:
...
if __name__ == '__main__':
    # training policy
    parameters = "data_set_sizes-per-Tanh-6-3102"
    path = "../data_set_sizes/"
    print(parameters)
